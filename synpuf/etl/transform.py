            ...
            if len(flds) == OMOP_CONCEPT_RECORD.fieldCount:
                ...
                if concept_id != '':
                    if vocabulary_id in [OMOP_CONSTANTS.ICD_9_VOCAB_ID,
                        ...
                        if concept_id not in concept_relationship_dict:
                            if domain_id not in domain_destination_file_list:
                            ...
                            for concept in concept_relationship_dict[concept_id]:
                                if concept not in domain_dict:
                                    domain_dict[concept] = None
                                    status = "No domain defined for concept " + concept
                                else:
...
def write_person_record(beneficiary):
    ...
    if int(yd.BENE_RACE_CD) == 1:    # White     
