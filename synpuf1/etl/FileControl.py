...
class FileDescriptor(object):
    ...
    def get_patient_records(self, DESYNPUF_ID, record_list):
        ...
        while True:
            try:
                ...
                if rec.startswith(DESYNPUF_ID.encode()):
                    record_list.append(rec.rstrip().decode().split(','))
                elif rec.startswith(DESYNPUF_ID.encode()) is False and rec < DESYNPUF_ID.encode():
                    pass
                else:
    ...
    class MultiThreadedGetPatientRecords:
        def __init__(self, fd, DESYNPUF_ID, record_list):
            self.thread = threading.Thread(target=get_patient_records, args=(fd, DESYNPUF_ID, record_list))
            self.thread.start()
